...
describe('MemorialService', () => {
  ...
  beforeEach(async () => {
    ...
    exileRepository = module.get<Repository<ExileStory>>(getRepositoryToken(ExileStory));
  });
  ...
  describe('Victims', () => {
    describe('findAllVictims', () => {
      it('should return paginated victims', async () => {
        ...
        jest.spyOn(victimRepository, 'createQueryBuilder').mockReturnValue(mockQb as any);
      ...
      it('should filter victims by year range', async () => {
        ...
        await service.findAllVictims({ yearFrom: 2017, yearTo: 2017, page: 1, limit: 20 });

        expect(mockQb.andWhere).toHaveBeenCalledWith(
          expect.stringContaining('EXTRACT(YEAR FROM victim.dateOfDeath) >= :yearFrom'),
          { yearFrom: 2017 },
        );
      });
    ...
    describe('findOneVictim', () => {
      ...
      it('should throw NotFoundException if victim not found', async () => {
        ...
        await expect(service.findOneVictim('invalid-id')).rejects.toThrow(NotFoundException);
      });
    });
  ...
  describe('Political Prisoners', () => {
    describe('findAllPrisoners', () => {
      it('should return paginated prisoners', async () => {
        ...
        jest.spyOn(prisonerRepository, 'createQueryBuilder').mockReturnValue(mockQb as any);
      ...
      it('should search prisoners by name', async () => {
        ...
        await service.findAllPrisoners({ search: 'Javier', page: 1, limit: 20 });
    ...
    describe('findOnePrisoner', () => {
      it('should return a prisoner by id', async () => {
        jest.spyOn(prisonerRepository, 'findOne').mockResolvedValue(mockPrisoner);
      ...
      it('should throw NotFoundException if prisoner not found', async () => {
        ...
        await expect(service.findOnePrisoner('invalid-id')).rejects.toThrow(NotFoundException);
      });
    });
  ...
  describe('Exile Stories', () => {
    describe('findAllExileStories', () => {
      it('should return paginated exile stories', async () => {
        ...
        jest.spyOn(exileRepository, 'createQueryBuilder').mockReturnValue(mockQb as any);

        const result = await service.findAllExileStories({ page: 1, limit: 20 });
    ...
    describe('findOneExileStory', () => {
      ...
      it('should throw NotFoundException if exile story not found', async () => {
        ...
        await expect(service.findOneExileStory('invalid-id')).rejects.toThrow(NotFoundException);
      });
    });
    ...
    describe('getMemorialStats', () => {
      it('should return memorial statistics', async () => {
        const mockStats = {
          ...
          totalPrisoners: 50,
          totalExiles: 7000000,
          victimsByYear: { 2017: 43, 2024: 200 },
        };
