...
export class OfacImporterService {
      ...
      for (const entry of venezuelanOfficials) {
        ...
        if (existing) {
          ...
          if (
            ...
            existing.dateOfBirth = entry.dob ? new Date(entry.dob) : undefined;
    ...
    return [
      {
        ...
        aka: ["Nicolas Maduro", "Maduro Moros", "Nicolas Ernesto Maduro Moros"],
        nationality: "VE",
        dob: "1962-11-23",
        remarks:
  ...
  async getTier1Stats(): Promise<{
    ...
    all.forEach((official) => {
      ...
      byTier[official.tier.toString()] =
        (byTier[official.tier.toString()] || 0) + 1;
    });
